...
Class: CPSC 3600
Assignment: Advanced Project - Implementing a Distributed Server Network
...
##############################################################################################################
    ...
    def setup_server_socket(self):
        ...
        try:
            ...
            self.print_info('Error at ' + str(self.port))
            return
        ...
        self.sel.register(self.server, event, temp)
    ...
    def connect_to_server(self):
        ...
        temp = ServerConnectionData(self.id, self.server_name, self.server_info)
        
        temp.write_buffer = b''

        event = selectors.EVENT_READ | selectors.EVENT_WRITE

        message = ServerRegistrationMessage.bytes(self.id, 0, self.server_name, self.server_info)

        temp.write_buffer += message

        self.sel.register(self.client_socket, event, temp)
    ...
    def accept_new_connection(self, io_device):
        ...
        connection, addr = io_device.fileobj.accept()
        
        connection.setblocking(False)

        temp = BaseConnectionData()
        ...
        self.sel.register(connection, event, temp)
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        if event_mask & selectors.EVENT_WRITE:
            if (io_device.data.write_buffer):
                x.send(io_device.data.write_buffer)
        ...
        if event_mask & selectors.EVENT_READ:
            ...
            if temp:
                self.handle_messages(io_device, temp)
            else:
                self.sel.unregister(x)
                x.close()
        pass
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        data = ServerConnectionData(message.source_id, message.server_name, message.server_info)

        data.first_link_id = message.last_hop_id

        if message.last_hop_id == self.id or message.last_hop_id == 0:
            data.first_link_id = message.source_id

            self.sel.modify(io_device.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, data)

        if message.last_hop_id == 0:
            reg = ServerRegistrationMessage.bytes(self.id, message.source_id, self.server_name, self.server_info)
            data.write_buffer = data.write_buffer + reg

            for conn in self.hosts_db.values():
                if isinstance(conn, ServerConnectionData):
                    reg_message = ServerRegistrationMessage.bytes(conn.id, self.id, conn.server_name, conn.server_info)

                    data.write_buffer = data.write_buffer + reg_message
                elif isinstance(conn, ClientConnectionData):
                    reg_client = ClientRegistrationMessage.bytes(conn.id, self.id, conn.client_name, conn.client_info)

                    data.write_buffer = data.write_buffer + reg_client

        self.hosts_db[message.source_id] = data

        temp2 = ServerRegistrationMessage.bytes(message.source_id, self.id, message.server_name, message.server_info)

        self.broadcast_message_to_servers(temp2, ignore_host_id = message.last_hop_id)
...
##############################################################################################################
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            status = StatusUpdateMessage.bytes(self.id, 0, 0x02, "A machine already registered with ID " + str(message.source_id))
            
            io_device.data.write_buffer = io_device.data.write_buffer + status
            return
        
        client = ClientConnectionData(message.source_id, message.client_name, message.client_info)
        
        client.first_link_id = message.last_hop_id

        if message.last_hop_id == 0 or message.last_hop_id == self.id:
            client.first_link_id = message.source_id
            self.sel.modify(io_device.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, client)

        if message.last_hop_id == 0:
            new_message = StatusUpdateMessage.bytes(self.id, message.source_id, 0x00, "Welcome to the Clemson Relay Chat network " + str(client.client_name))
            client.write_buffer = client.write_buffer + new_message
        
            for conn in self.hosts_db.values():
                if isinstance(conn, ClientConnectionData):
                    client_reg = ClientRegistrationMessage.bytes(conn.id, self.id, conn.client_name, conn.client_info)
                    
                    client.write_buffer = client.write_buffer + client_reg
        
        self.hosts_db[message.source_id] = client
        
        temp2 = ClientRegistrationMessage.bytes(message.source_id, self.id, message.client_name, message.client_info)
        
        self.broadcast_message_to_servers(temp2, ignore_host_id = message.last_hop_id)
        self.broadcast_message_to_adjacent_clients(temp2, ignore_host_id = message.last_hop_id)
...
##############################################################################################################
    ...
    def handle_client_chat_message(self, io_device, message):
        ...
        if message.destination_id in self.hosts_db:
            ...
            status = StatusUpdateMessage.bytes(self.id, 0, 0x01, "Unknown ID " + str(message.destination_id))

            io_device.data.write_buffer = io_device.data.write_buffer + StatusUpdateMessage.bytes(self.id, 0, 0x01, "Unknown ID " + str(message.destination_id))
        pass
